...
class ToolUser:
    ...
    def __init__(self, tools, api_key, temperature=0, max_retries=3, first_party=True, model="default"):
        self.tools = tools
    ...
    def _convert_value(value, type_str):
        ...
        """
        if type_str == "bool":
            # Explicitly handle boolean type conversion to interpret "True" and "False" strings correctly
            return value.lower() == "true"
        elif type_str in ("list", "dict"):
            # Use ast.literal_eval for complex types
            return ast.literal_eval(value)
        
        # For other types, use the built-in type conversion
        type_class = getattr(builtins, type_str, None)
        if type_class is not None:
            try:
                return type_class(value)
            except ValueError:
                pass
        # Return the value unchanged if conversion is not possible
        return value
